...
std::tuple<int, int, int, int> Grid::getBin(Coordinate c)
{
    int binx = (int)((c.x + gridSize.x*0.5) / (double)binSize.x);
	int biny = (int)((c.y + gridSize.y*0.5) / (double)binSize.y);
	int binz = (int)((c.z + gridSize.z*0.5) / (double)binSize.z);
    ...
    // move this particle inside the closest bin in each dimension if necessary
    if(binx < 0)
    {
        binx = 0;
    }
	...
	return std::make_tuple(bin, binx, biny, binz);
    //return bin;
...
std::vector<int> Grid::getBinsInRegion(Coordinate centerPosition, double radius)
{
    ...
    std::tuple <int,int,int,int> binTuple = getBin(centerPosition);
    int centralBin = std::get<0>(binTuple);
    
    int binx = std::get<1>(binTuple);
    int biny = std::get<2>(binTuple);
    int binz = std::get<3>(binTuple);
...
std::vector<int> Grid::getTagsBetweenZVals(double zPosLow, double zPosHigh)
{
    int highestZBin = (int)(-zPosHigh / (double)binSize.z);
    int lowestZBin = (int)(-zPosLow / (double)binSize.z);
    
    std::vector<int> particleTags;
    
    while(highestZBin >= lowestZBin)
    {
        for(int i = 0; i < numBinX*numBinY; i++)
        {
            int currBin = highestZBin + i*numBinZ;
